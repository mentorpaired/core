...
class TestRequestInterestViews(BaseTestCase):
    ...
    def test_retrieve_valid_single_interest(self):
        ...
        interest = RequestInterest.objects.get(pk=self.request_interest.interest_id)
    ...
    def test_retrieve_all_interests_linked_to_single_request(self):
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + self.token)
        response = self.client.get(f"/requests/{self.request.request_id}/interests/")
        self.assertEqual(response.status_code, 200)
    ...
    def test_can_update_field_in_interest(self):
        ...
        response = self.client.patch(
            f"/interests/{self.request_interest.request_id}/",
            {
    ...
    def test_can_delete_interest(self):
        self.client.credentials(HTTP_AUTHORIZATION="Bearer " + self.token)
        response = self.client.delete(f"/interests/{self.request_interest.pk}/")
